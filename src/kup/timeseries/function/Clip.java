...
public final class Clip extends FunctionBase implements Function {
	private final float min;
	private final float max;

	public Clip(TimeSeries input, float min, float max) {
		this.min = min;
		this.max = max;
		if (max < min) {
			max = min;
	...
	String describeArgs() { return String.valueOf(min) + " , " + String.valueOf(max); }
