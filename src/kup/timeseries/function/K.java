...
public final class K extends FunctionBase implements Function {
	private final float k;

	public K(TimeSeries input, float k) {
		this.k = k;
		setInput(input);
	}
	
	String describeArgs() { return String.valueOf(k); }
	
	DataPoint computeNext(DataPoint in) {
		return new DataPoint(in.date(), in.value() * k);
	}
}
