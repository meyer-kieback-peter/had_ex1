...
public final class Taper extends FunctionBase implements Function {
	private final float delta;
	private DataPoint last;

	public Taper(TimeSeries input, float t) {
		this.delta = t;
		setInput(input);
	}
	
	String describeArgs() { return String.valueOf(delta); }
	...
	DataPoint computeNext(DataPoint in) {
		...
		if (Math.abs(last.value() - in.value()) >= delta) {
			last = in;
			return in;
		}
		return null;
