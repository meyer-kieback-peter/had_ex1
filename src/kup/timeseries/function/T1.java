...
public final class T1 extends FunctionBase implements Function {
	private final float t;
	private float s = 0.0f;
	private DataPoint last;

	public T1(TimeSeries input, float t) {
		this.t = t;
		setInput(input);
	}
	
	String describeArgs() { return String.valueOf(t); }
	
	DataPoint computeNext(DataPoint in) {
		if (null == last) {
			s = in.value();
			last = in;
			return new DataPoint(in.date(), s);
		}
		float dtsecs = (in.getTime() - last.getTime()) / 1000.0f;
		float v = s + (in.value() - s) * (1.0f - (float)Math.exp(-1.0f * dtsecs / t));
		s = v;
		return new DataPoint(in.date(), v);
	}
}
