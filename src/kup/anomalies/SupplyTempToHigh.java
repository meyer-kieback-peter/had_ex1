...
/**
 * HVAC anomaly: supply temperature way above setpoint - control or plant problems.
 * 
 * This anomaly is detected, if the supply temperature exceed
 ...
 * limit - detection limit for anomaly situation 0 .. 1 (0 .. 100% ), useful default is 0.15 (15% above setpoint) 
 */
public class SupplyTempToHigh {
	...
	public static TimeSeries compute(TimeSeries tSup, TimeSeries tSupSetpt, float t1, float limit) throws CrossSampleException {
		...
		Clip clippedDeviation = new Clip(deviation.compute(), 0.0f, Float.POSITIVE_INFINITY);
		K maxDeviation = new K(tSupSetptT1.compute(), limit);
		GreaterOrEqual anomalyUntapered = new GreaterOrEqual(clippedDeviation.compute(), maxDeviation.compute());
